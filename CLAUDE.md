...
| `image_field` | no | Which field to expose as `page.social_image` (Open Graph/Twitter Card image, see "SEO, sitemap, robots.txt, 404" below), if not `image`. |
...
- **Assets** (`Contentful::Asset`) → `{ "url", "content_type", "title", "description", "width", "height" }`, with `url` rewritten from Contentful's protocol-relative form (`//images.ctfassets.net/...`) to an absolute `https://` URL. Use as `<img src="{{ page.cover_image.url }}" width="{{ page.cover_image.width }}" height="{{ page.cover_image.height }}">` (explicit dimensions avoid layout shift as the image loads). An asset that hasn't finished processing yet (e.g. linked from a draft entry via `CONTENTFUL_PREVIEW`) has no `url`/`content_type`/`details` key in its raw `file` JSON at all — contentful.rb only defines a method per key actually present, so an absent one is a genuinely undefined method, not `nil` (`file&.url` doesn't help — `&.` only guards a nil receiver). `#serialize_asset`'s `safe_field` helper checks `respond_to?` before calling each one rather than assuming a non-nil `file` always has all three. `width`/`height` come from `file.details["image"]` — a plain parsed-JSON Hash, not another dynamic contentful.rb object, so a non-image asset (a PDF, say) or one still processing just reads as `nil` there, no error.
...
- **Depth capping**: recurses up to `contentful_entry_depth` levels (default `EntrySerializer::DEFAULT_ENTRY_DEPTH`, 2) to bound build output on heavily cross-referenced content models — reconstructing a nested tree from Contentful's flat, deduplicated `includes` list means a shared reference gets fully duplicated per occurrence in the output, so uncapped recursion risks real bloat on content models with wide fan-out (e.g. "related posts"). Beyond that depth, and for any link Contentful didn't resolve at all, an entry degrades to `{ "id", "link_type" }` — the same shape as an unresolved `Contentful::Link` stub. Content models with deep reference chains (e.g. a `menu` entry linking `page`s which each link their own `subpages`, needing depth 3) should raise `contentful_entry_depth` in `_config.yml` accordingly.
...
- Sets `page.data["locale"]` to the locale code for every non-primary page (never for the primary locale, matching the `nil` "no locale in play" convention used everywhere else) — this is what `_layouts/default.html`'s nav and `index.html`'s homepage listing use to scope themselves to one locale at a time via `where: "locale", page.locale`, a true no-op when every page's `locale` is `nil`. This relies specifically on **Jekyll's own** `where` filter override (`Jekyll::Filters#where`, via `compare_property_vs_target`'s explicit `when NilClass; return true if property.nil?`), not raw Liquid's `StandardFilters#where` (which has a different, truthy-only nil behavior) — the two are easy to conflate. Like `nav`/`home_label`, a Contentful field literally named `locale` on some content type would silently collide and overwrite this (same class of risk noted below in "Field exposure" for `home_label`/body field names).
...
`each_locale`'s URL/data-key prefix only covers the locale code itself. Two collection settings can additionally be a Hash keyed by locale code instead of one plain value, for content that needs translating beyond what the prefix covers: `dir` (`ContentfulJekyll.dir_for`, resolved once per collection per locale pass in `fetch_collection` since it can't vary per entry), for when the URL path segment itself needs translating (e.g. `produkter` vs `products`), not just prefixing; and `label` (`home_label_for`), for a translated homepage section heading. They differ in failure mode on purpose: `.dir_for` raises immediately if a configured locale is missing from its Hash (a broken/empty URL segment is a real defect), while `home_label_for` falls back to the humanized default instead (an untranslated heading still works, it's just not localized yet). A locale code has to already exist in the Contentful space (Settings → Locales) before it can be queried — the CDA returns a 400 ("Unknown locale") for one that isn't, with no fallback.
...
No feed is shipped by default, and `jekyll-feed` is deliberately not a dependency: its bundled feed template assumes blog-post-shaped fields (a plain string `image`, a `date`, etc.) and crashes as soon as a Contentful field shares one of those names with a different shape — e.g. this template's `image` field is always a serialized asset Hash, not a plain path. Since this template's entire point is an arbitrary, per-site content model, no field-name convention is safe to assume.

Most sites built from this template won't have blog/news-shaped content anyway (a feed is genuinely useful for chronological, article-like content, not a product catalog). If a specific site does need a feed, don't reach for `jekyll-feed` — write a small hand-rolled one instead, following the exact pattern `index.html` already demonstrates: loop `site.pages`, filter by whatever flag marks feed-worthy content (mirroring `home`/`nav`), and read only `title`/`url`/`content` (already-rendered HTML, available on any page from anywhere once Jekyll's content-conversion phase has run) — fields every generated page already has, regardless of content type.

### SEO, sitemap, robots.txt, 404

- **`_includes/seo.html`**, included from `_layouts/default.html`'s `<head>`, emits a canonical link, a meta description, Open Graph tags, and Twitter Card tags. It reads only fields every page already has (`page.title`/`site.title`, `page.url`, the rendered `content`) or a generic alias set by the generator, the same "don't assume a site-specific field name" reasoning as the feed removal above:
  - `meta_description`/`og:description`/`twitter:description`: `page.description` if a content type happens to have one, else `content | strip_html | normalize_whitespace | truncate: 160` — works for any content type, since every generated page has rendered `content`.
  - `og:image`/`twitter:image`: `page.social_image.url`, set by `EntriesGenerator#build_page` as `page.data[collection["image_field"] || "image"]` — a generic alias for whatever field holds a collection's social preview image, since content types name it differently (`coverImage`, `image`, `contentImage`, ...). Configure `image_field` per collection (see the `contentful_collections` table above) if it isn't literally `image`; harmless (just `nil`, same as any other absent field) if no such field exists. `twitter:card` switches between `summary_large_image` and `summary` based on whether it's present.
  - `og:locale`: `page.locale` (falling back to `site.lang`, same as `<html lang>`) with `-` replaced by `_` — Contentful/BCP-47 locale codes are hyphenated (`nb-NO`), Open Graph's convention is underscored (`nb_NO`).
  - `og:type`: `article` if `page.home_label` is set (i.e. this page belongs to a `home: true` collection — see "Homepage & navigation"), else `website`.
- **`sitemap.xml`**/**`robots.txt`** (root, both `layout: none`) are hand-rolled rather than using the `jekyll-sitemap` gem, for the same reason `jekyll-feed` isn't a dependency (see above): a sitemap only needs each page's `.url`, so writing it directly is simpler and safer than depending on a third-party gem's own assumptions about this template's arbitrary content model. `sitemap.xml` loops `site.pages | where_exp` filtering out `404.html`/`sitemap.xml`/`robots.txt` by `.name` (both because they have front matter, they're Jekyll pages too, and would otherwise list themselves) and prints `<loc>{{ p.url | absolute_url }}</loc>` for the rest — this works identically for static pages and every Contentful-generated page (locale-prefixed URLs included, since those are just the page's `.url`). `robots.txt` allows everything and points at `sitemap.xml` via `absolute_url`.
- **`404.html`** (root, `layout: default`) is a plain static page with a link back to `/`. No `permalink:` override is needed — Jekyll already outputs a page named `404.html` at `/404.html` by default, which is what GitHub Pages requires to serve it as the 404 response.
- All of the above need `_config.yml`'s `url` set to the site's real deployed URL (e.g. `https://<user>.github.io`) — `absolute_url`/`| absolute_url` render relative/empty otherwise, which breaks canonical links, Open Graph tags, and the sitemap alike.
